...
#### Before You Download...
*This program is OUTDATED!* You should instead use [EzRO](https://github.com/Mips96/EzRO-gui), a GUI-based rom organizer that is intended to be this program's successor. It is much easier to use, includes a few new organization features, and fixes a few bugs that I didn't originally catch (specifically, Rom Organizer 3 has a bug that occurs if you audit a romset that includes two copies of the same rom (the same _rom_, not the same game); this is an extremely rare situation, but sorry if you run into it!).
